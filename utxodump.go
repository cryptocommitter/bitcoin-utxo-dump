...
func getBytePrefix(networkType string, testnet bool, addressType string) byte {
				...
				if fieldsSelected["address"] || fieldsSelected["type"] {
					...
					case nsize == 0:
						if fieldsSelected["address"] { // only work out addresses if they're wanted
							address = keys.Hash160ToAddress(script, []byte{getBytePrefix(*network, testnet, "p2pkh")}) // (m/n)address - testnet addresses have a special prefix
						}
						scriptType = "p2pkh"
					...
					case nsize == 1:
						if fieldsSelected["address"] { // only work out addresses if they're wanted
							address = keys.Hash160ToAddress(script, []byte{getBytePrefix(*network, testnet, "p2sh")})
						}
						scriptType = "p2sh"
					...
					case 1 < nsize && nsize < 6: // 2, 3, 4, 5
						...
						if fieldsSelected["address"] { // only work out addresses if they're wanted
							if *p2pkaddresses { // if we want to convert public keys in P2PK scripts to their corresponding addresses (even though they technically don't have addresses)
								...
								address = keys.PublicKeyToAddress(script, []byte{getBytePrefix(*network, testnet, "p2pk")})
							}
						}
					...
					case nsize == 28 && script[0] == 0 && script[1] == 20: // P2WPKH (script type is 28, which means length of script is 22 bytes)
						...
						if fieldsSelected["address"] { // only work out addresses if they're wanted
							if testnet == true {
								...
								fmt.Printf("Got the following: %s, %t, %d", network, testnet, version)
					...
					case nsize == 40 && script[0] == 0 && script[1] == 32: // P2WSH (script type is 40, which means length of script is 34 bytes; 0x00 means segwit v0)
						...
						if fieldsSelected["address"] { // only work out addresses if they're wanted
							if testnet == true {
								...
								if address == "bc1qd5wm03t5kcdupjuyq5jffpuacnaqahvfsdu8smf8z0u0pqdqpatqnfdraj" {
									fmt.Printf("Got pre")
								}
							}
						}
